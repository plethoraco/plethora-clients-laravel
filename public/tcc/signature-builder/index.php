	...
	<div id="builder">
		<form method="post" enctype="multipart/form-data">
			...
			<fieldset>
				...
				<input type="text" name="phone" id="phone" value="<?=v('phone')?>">
			</fieldset>
			<fieldset>
			...
			*/ ?>
			<fieldset>
				<label for="template">School</label>
				<select name="template" id="template"><?
					foreach ($GLOBALS['settings']['themes'] as $id => $template) {
						?><option value="<?=$id?>" <?=(v('template') == $id) ? ' selected="selected"' : ''?>><?=$template['name']?></option><?
					}
				?></select>
			</fieldset>
			<fieldset>
				...
				<? foreach ($GLOBALS['settings']['themes'] as $id => $template) {
					?><select id="address_<?=$id?>" disabled="disabled" style="display:none;"><?
						foreach ($template['addresses'] as $address) {
							?><option value="<?=$address['address']?>" <?=(stringsEqual(v('address'), $address['address'], true)) ? ' selected="selected"' : ''?>><?=$address['name']?></option><?
						}
					?></select><?
				} ?>
			</fieldset>
